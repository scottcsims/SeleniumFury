...
describe PageObject do
  ...
  describe '#element' do
    context 'when timeout is not defined' do
      it 'should raise an exception when the element cannot be found' do
        launch_web_driver TEST_PAGE_URL
        test_page = TestPage.new(driver)
        TestPage.element :not_a_element, {id: 'not a element'}
        expect { test_page.not_a_element }.to raise_exception RuntimeError, 'Could not find element not_a_element (Waited 0.5 seconds.)'
      end
    end
    context 'when timeout is defined' do
      it 'should raise an exception when the element cannot be found, the time should be shown in the exception' do
        launch_web_driver TEST_PAGE_URL
        test_page = TestPage.new(driver)
        TestPage.element :cats, { id: 'meow' }, {timeout: 2}
        expect { test_page.cats }.to raise_error RuntimeError, 'Could not find element cats (Waited 2 seconds.)'
      end
    end
  end
  ...
  describe '#element_list' do
    ...
    context 'when timeout is defined' do
      it 'and element_list does not exist, the time should be shown in the exception' do
        launch_web_driver TEST_PAGE_URL
        test_page = TestPage.new(driver)
        TestPage.element_list :cats, { id: 'meow' }, {timeout: 2}
        expect { test_page.cats }.to raise_error RuntimeError, 'Could not find any elements like cats (Waited 2 seconds.)'
      end
    end
  end
end
